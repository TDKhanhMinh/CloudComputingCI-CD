...
import com.example.Final.service.Payment.VNPayPayment;
...
class VNPayPaymentTest {
    ...
    void testCreateUserPayment() {
        double amount = 500.0;
        vnPayPayment.createUserPayment(user, amount);

        assertEquals(1450.0, user.getAccountBalance(), 0.01);
        ...
        assertEquals("vnpay", paymentDetails.getPaymentMethod());
        assertEquals(450.0, paymentDetails.getAmount(), 0.01);
